...
def KNNTimeVSelectivity():
    ...
    for k in k_list:
        for s in s_list:
            if (f"{k}" in knn_qtest_output["knn-q1"][f"{s}"]):
                d1l1.append(float(s))
                d1l2.append(float(knn_qtest_output["knn-q1"][f"{s}"][f"{k}"]["total_time"]))
                d1l3.append(f"k_value: {k}")           


    d1table = {'Time (ms)': d1l2, 'Selectivity': d1l1, 'k': d1l3}
...
def TimeVSelectivityFixedEF(ef_sch):
    ...
    for k in k_list:
        for s in s_list:
            if (f"{k}" in kann_qtest_output["ann-qtest"]["ann-q1"][str(ef_sch)][f"{s}"]):
                d1l1.append(float(s))
                d1l2.append(float(kann_qtest_output["ann-qtest"]["ann-q1"][str(ef_sch)][f"{s}"][f"{k}"]["total_time"]))
                d1l3.append(f"k: {k}")
...
def RecallVSelectivityFixedEF(ef_sch):
    ...
    for k in k_list:
        for s in s_list:
            if (f"{k}" in knn_qtest_output["knn-q1"][f"{s}"]):
                string_with_coordinates1 = knn_qtest_output["knn-q1"][f"{s}"][f"{k}"]["nearest_neighbors"]
                ...
                string_with_coordinates2 = kann_qtest_output["ann-qtest"]["ann-q1"][str(ef_sch)][f"{s}"][f"{k}"]["nearest_neighbors"]
...
def TimeVefFixedSelectivity(selectivity):
    ...
    for k in k_list:
        for ef in ef_list:
            if (f"{k}" in kann_qtest_output["ann-qtest"]["ann-q1"][f"{ef}"][f"{selectivity}"]):
                d1l1.append(float(ef))
                d1l2.append(float(kann_qtest_output["ann-qtest"]["ann-q1"][f"{ef}"][f"{selectivity}"][f"{k}"]["total_time"]))
                d1l3.append(f"k: {k}")
...
def RecallVefFixedSelectivity(selectivity):
    ...
    for k in k_list:
        for ef in ef_list:
            if (f"{k}" in knn_qtest_output["knn-q1"][f"{selectivity}"]):
                string_with_coordinates1 = knn_qtest_output["knn-q1"][f"{selectivity}"][f"{k}"]["nearest_neighbors"]
                # Extract x coordinates using regular expressions
                x_coordinates1 = re.findall(r"\(([0-9]+),", string_with_coordinates1)
                x_coordinates_list1 = [int(x) for x in x_coordinates1]
                # print(s, k)
                # print(x_coordinates_list1)
                string_with_coordinates2 = kann_qtest_output["ann-qtest"]["ann-q1"][str(ef)][f"{selectivity}"][f"{k}"]["nearest_neighbors"]
                # print(string_with_coordinates1)
                # print(string_with_coordinates2)
                x_coordinates2 = re.findall(r"\(([0-9]+),", string_with_coordinates2)
                x_coordinates_list2 = [int(x) for x in x_coordinates2]
...
fig = KNNTimeVSelectivity()
filename = "images/KNNTimeVSelectivity.png"
